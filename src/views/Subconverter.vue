<template>
  <div>
            ...
            >
              ...
              <div v-if="advanced === '2'">
                ...
                <el-form-item label="Include:">
                  ...
                  <div class="keywords-tags" style="margin-top: 10px">
                    <el-tag
    ...
    <el-dialog
      title="添加包含关键字"
      :visible.sync="includeDialogVisible"
      width="30%"
    >
      <el-input
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click="includeDialogVisible = false">取 消</el-button>
        <el-button type="primary" @click="confirmAddIncludeKeyword"
          >确 定</el-button
        >
      </span>
    </el-dialog>
    ...
    >
      ...
      ></el-input>
      <span slot="footer" class="dialog-footer">
        <el-button @click="excludeDialogVisible = false">取 消</el-button>
        <el-button type="primary" @click="confirmAddExcludeKeyword"
          >确 定</el-button
        >
      </span>
    </el-dialog>
...
export default {
  data() {
    return {
      ...
      options: {
        ...
        // 读取 .env 中的 VUE_APP_SUB_BACKENDS
        // 将字符串按逗号分割成数组，并转换成对象格式
        backendOptions: process.env.VUE_APP_SUB_BACKENDS.split(",").map(
          (url) => ({
            value: url,
          })
        ),
        remoteConfig: [
          {
            ...
            options: [
              ...
              {
                label: "ACL4SSR_NoAuto_NoApple 本地 无自动测速&苹果分流",
      ...
      includeDialogVisible: false,
      newIncludeKeyword: "",
      includeKeywords: [], // 存储包含关键字数组
      excludeKeywords: [], // 存储排除关键字数组
      newExcludeKeyword: "",
  ...
  methods: {
    ...
    addIncludeKeyword() {
      this.includeDialogVisible = true;
      this.newIncludeKeyword = "";
    },
    confirmAddIncludeKeyword() {
      if (this.newIncludeKeyword.trim()) {
        this.includeKeywords.push(this.newIncludeKeyword.trim());
        this.updateIncludeRemarks();
        this.includeDialogVisible = false;
      }
    },
    removeIncludeKeyword(index) {
      this.includeKeywords.splice(index, 1);
      this.updateIncludeRemarks();
    },
    updateIncludeRemarks() {
      if (this.includeKeywords.length === 0) {
        this.form.includeRemarks = "";
      } else {
        // 将关键字数组转换为正则表达式
        this.form.includeRemarks = this.includeKeywords
    ...
    confirmAddExcludeKeyword() {
      if (this.newExcludeKeyword.trim()) {
        this.excludeKeywords.push(this.newExcludeKeyword.trim());
        this.updateExcludeRemarks();
        this.excludeDialogVisible = false;
      }
    },
    removeExcludeKeyword(index) {
      this.excludeKeywords.splice(index, 1);
      this.updateExcludeRemarks();
    },
    updateExcludeRemarks() {
      if (this.excludeKeywords.length === 0) {
        this.form.excludeRemarks = "";
      } else {
        // 将关键字数组转换为正则表达式
        this.form.excludeRemarks = this.excludeKeywords
          .map((keyword) => this.escapeRegExp(keyword))
          .join("|");
      }
    },
